...
class ScanningUsersDAL:
    ...
    async def create_scanning_user(self, tg_id:int, username:str, **extra_fields) -> ScanningUser:
    ...
    async def update_user(self, tg_id:int, **kwargs) -> ScanningUser:
        query = (
            update(ScanningUser)
...
async def _get_scanning_user(tg_id:int):
    async with async_session() as session:
        async with session.begin():
            return (await session.execute(select(ScanningUser).filter_by(tg_id=tg_id))).scalar_one_or_none()


async def _create_new_scanning_user(tg_id: int, other_fields: dict):
    async with async_session() as session:
        async with session.begin():
            scanning_users_dal = ScanningUsersDAL(session)

            user = await scanning_users_dal.create_user(
                tg_id=tg_id,
                **other_fields
                )
...
async def _update_scanning_user(tg_id: int, updated_user_params: dict):
    async with async_session() as session:
        async with session.begin():
            scanning_users_dal = ScanningUsersDAL(session)
            updated_user = await scanning_users_dal.update_user(
                tg_id=tg_id, **updated_user_params
            )
            return ShowAndCreateScanningUser(
                tg_id=user.tg_id,
                is_scanning=user.is_scanning
...
async def set_if_scanning(tg_id: int, other_fields: dict):
    scanning_user = await _get_scanning_user(tg_id)
    
    if scanning_user:
        print("Пользователь найден")
        return await _update_scanning_user(tg_id=tg_id, updated_user_params=other_fields)
    else:
        print("Пользователь не найден")
        return await _create_new_scanning_user(tg_id=tg_id, other_fields=other_fields)
...
async def start(data:StartData):
    ...
    print(await _get_scanning_user(tg_id))
